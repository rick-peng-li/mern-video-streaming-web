...
export default function VideoEditPage() {
  ...
  // axios put the values to the backend
  const putToServer = async (videoInfo) => {
    const payload = {
      ...
      recordingDate: videoInfo.recordingDate,
    ...
    try {
      ...
      setUploadResponse('Video updated successfully');
      setTimeout(() => navigate('/videos'), 1500);
    } catch (error) {
      setAlertType('error');
      setUploadResponse(error.response?.data?.message || 'Update failed');
    }
  };
